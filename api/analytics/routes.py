...
def get_events_list():
    ...
    try:
        ...
        # Build query - exclude admin/analytics page events only
        query = AnalyticsEvent.query.filter(
            AnalyticsEvent.timestamp >= start_time
        ).filter(
            ~AnalyticsEvent.page_url.like('%/admin/analytics%')
        )
        ...
        # Get unique event types for filtering (exclude admin/analytics only)
...
def get_dashboard():
    ...
    try:
        ...
        # Total page views (exclude admin/analytics)
        total_page_views = PageView.query.filter(
            PageView.timestamp >= start_time
        ).filter(
            ~PageView.page_url.like('%/admin/analytics%')
        ).count()
        
        # Total events (exclude admin/analytics only)
        ...
        # Top events (exclude admin/analytics)
        ...
        # Recent activity (last 100 events) with detailed descriptions (exclude admin/analytics)
        ...
        # Error rate (events with error in name or type, exclude admin/analytics)
        ...
        # Conversion rate (events with conversion in name, exclude admin/analytics)
