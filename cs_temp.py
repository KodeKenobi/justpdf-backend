...
def _user_friendly_message(level, action, message):
    ...
    if level == 'info':
        ...
        if 'contact page' in msg_lower and ('scroll' in msg_lower or 'wait' in msg_lower):
            return "Loading contact pageΓÇª"
        if 'testing link' in msg_lower:
            return "Checking a linkΓÇª"
        if 'discovery' in msg_lower:
            return "Searching for contact optionsΓÇª"
        if 'sending email' in msg_lower:
            return "Sending emailΓÇª"
    # Warnings
    if level == 'warning':
    ...
    if message and len(message) > 80:
        ...
        short = short[:77] + 'ΓÇª' if len(short) > 80 else short
        return short
    return message or "ProcessingΓÇª"
